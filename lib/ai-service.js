...
async function callOpenAI(promptOrMessages, model, maxTokens, retries, timeoutMs) {
  ...
  while (retries > 0) {
    try {
      ...
      const modelName = model || process.env.OPENAI_MODEL || "gpt-4-turbo-preview";
      
      // Some newer models (o1, o3, GPT-5.2+) require max_completion_tokens instead of max_tokens
      ...
      // Some models (o1, o3) don't support temperature parameter - must use default (1) or omit it
      const doesNotSupportTemperature = 
        modelName.startsWith('o1') || 
        modelName.startsWith('o3') ||
        modelName.includes('o1-') ||
        modelName.includes('o3-');
      ...
      // Check if error is about max_tokens vs max_completion_tokens
      if (err.message && err.message.includes("max_completion_tokens") && err.message.includes("max_tokens")) {
        // Retry with max_completion_tokens instead - rebuild apiParams
        try {
          let messages;
          let systemPrompt = null;
          ...
          // If retry also fails, fall through to normal retry logic
          retries--;
      ...
      else if (err.message && err.message.includes("temperature") && err.message.includes("not support")) {
        // Retry without temperature parameter - rebuild apiParams
        try {
          let messages;
          let systemPrompt = null;
          ...
          // Don't include temperature for models that don't support it
          // Use the appropriate max tokens parameter
          if (requiresMaxCompletionTokens) {
            apiParams.max_completion_tokens = maxTokens;
          } else {
            apiParams.max_tokens = maxTokens;
